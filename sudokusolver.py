...
# ============================================================================
# PUZZLE BANK (Categorized by Difficulty)
# ============================================================================
...
RAW_PUZZLE_BANK = {
    ...
    'Expert': [
        ...
        '..........5.3.7...8.1....6............2....4.5.....9..2.....1...4......3.6...9...',
        '..............3.85..1.2.......5.7.....4...1...9.......5......73..2.1........4...9',
    ]
}

def sanitize_puzzle_bank(raw_dict):
    """Sanitize puzzle bank dictionary"""
    PUZZLE_BANK = []
    ...
    return PUZZLE_BANK if PUZZLE_BANK else [("empty", "." * 81)]
        ...
        for i in range(9):
            for j in range(9):
                ...
                if ch.isdigit() and ch != '0':
                    ...
                    bit = 1 << (v-1)
                    bidx = (i//3)*3 + (j//3)
                    if (self.row_masks[i] & bit) or (self.col_masks[j] & bit) or (self.box_masks[bidx] & bit):
                        raise ValueError(f"Initial conflict at ({i},{j})")
                    self.board[i][j] = v
...
class SudokuFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.play_bg = wx.Colour(240, 250, 248)  # Very light teal
        self.solver_bg = wx.Colour(245, 243, 255)  # Very light purple
        self.box_colors_play = [
            wx.Colour(224, 242, 241),  # Light teal
            wx.Colour(248, 252, 252),  # Almost white with teal hint
        ]
        self.box_colors_solver = [
            wx.Colour(238, 233, 255),  # Light lavender
            wx.Colour(248, 248, 255),  # Almost white with purple hint
        ]
    ...
    def _init_play_controls(self, parent, sizer):
        """Initialize play mode controls - COMPACT BUTTONS"""
        ...
        self.timer_label = wx.StaticText(self.play_panel, label="⏱️ 00:00")
        ...
        # Navigation buttons - COMPACT SIZE (28px height)
        ...
        # Action buttons - COMPACT SIZE (32px height)
        ...
        # Check solution button - NORMAL SIZE (40px height)
        self.check_btn = wx.Button(self.play_panel, label="✓ CHECK SOLUTION", size=(-1, 40))
        self.check_btn.SetFont(wx.Font(11, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        self.check_btn.SetBackgroundColour(wx.Colour(32, 178, 170))
        self.check_btn.SetForegroundColour(wx.Colour(255, 255, 255))
        self.check_btn.Bind(wx.EVT_BUTTON, self._on_check)
        panel_sizer.Add(self.check_btn, 0, wx.EXPAND|wx.ALL, 5)
    ...
    def _on_text(self, event, row, col):
        ...
        try:
            ...
            # Auto-advance to next cell (only in play mode)
    ...
    def _validate_cell(self, row, col):
        ...
        try:
            ...
            # Valid placement - show success briefly
    ...
    def _on_check(self, event):
        ...
        for i in range(9):
            for j in range(9):
                value = self.cells[i][j].GetValue()
                if not value or not value.isdigit() or int(value) != self.solution[i][j]:
                    correct = False
                    break
            if not correct:
                break
        ...
        if correct:
            ...
            hints_used = self.max_hints - self.hints_remaining
            wx.MessageBox(
                f"🎉 Congratulations! 🎉\n\nTime: {elapsed}\nHints used: {hints_used}/{self.max_hints}",
                "Puzzle Solved!",
                wx.OK|wx.ICON_INFORMATION
            )
        else:
            wx.MessageBox("Not quite right yet. Keep trying!", "Check", wx.OK|wx.ICON_INFORMATION)
    ...
    def _update_timer(self):
        ...
        try:
            ...
            self.timer_label.SetLabel(f"⏱️ {self._format_time(self.elapsed_time)}")
